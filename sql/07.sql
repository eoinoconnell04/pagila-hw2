...
SELECT DISTINCT title FROM film JOIN inventory USING(film_id) WHERE film_id NOT IN (SELECT film_id FROM inventory JOIN rental USING (inventory_id) JOIN customer USING(customer_id) JOIN address USING(address_id) JOIN city USING(city_id) JOIN country USING(country_id) where country.country ILIKE 'United States') order by title; 
